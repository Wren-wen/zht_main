    ...
    <div class="configurationArticle">
        <div class="col">
            <div class="titlebar">
                ...
                <div @click="addnewFn" class="btn addnew nose pointer">找回/添加新文章 +</div>
            </div>
            <div class="bar">
                <div class="col1">
                    <div class="bar two">
                ...
                <div class="col1 two">
                    ...
                    <div class="content scrollBar right">
                        <transition-group name="opacity">
                            <article-item-mod @click="opt.id_selected = parseInt(key)" :value="val"
        ...
        <div class="content">
            <article-content v-if="opt.id_selected" :key="opt.id_selected"></article-content>
        </div>
    </div>
</template>
...
const INFO_TYPE = {
    0: '文件$file不存在读取失败, 编辑完成后点击下载按钮, 将文件$file放在目录[public/configuration/article]下',
    1: '文件$file存在变更, 点击下载按钮, 将文件$file放在目录[public/configuration/article]以保存',
    2: '文件$file已存在, 等待合并索引文件article_index.json 后以保存',
    3: '文件$file已是最新且在仓库索引article_index.json 中, 无需保存',
    4: '文件$file已标记为删除, 合并索引文件article_index.json 后生效',
}
...
const mergeFn = () => {
    if (has_unsave.value) {
        ...
        // 下载json
        download_json(merge_index, 'article_index')
    }
}
getIndex().then(res => {
    opt.index_init = res
