    ...
    <div class="configurationArticle">
        <div class="col">
            <div class="titlebar">
                ...
                <div @click="addnewFn" class="btn addnew nose pointer">找回/添加商品 +</div>
            </div>
            <div class="bar">
                <div class="col1">
                    <div class="bar two">
                ...
                <div class="col1 two">
                    ...
                    <div class="content scrollBar right">
                        <transition-group name="opacity">
                            <product-item-mod @click="opt.id_selected = parseInt(key)" :value="val"
            ...
            <div class="col two nose">
                ...
                <div @click="mergeFn" :class="{
                    ...
                    <div>2.下载合并的索引product_index</div>
                </div>
        ...
        <div class="content">
            <product-content v-if="opt.id_selected" :key="opt.id_selected"></product-content>
        </div>
    </div>
</template>
...
import { getIndex } from '@/api/configuration/product'
import { useRoute, useRouter, onBeforeRouteLeave } from "vue-router"

// 配置
// 索引中需要保留的字段
const INDEX_PARAM = ['id', 'cover', 'title', 'title_en', 'timestamp', 'path']
// 所有字段的初始化配置，id，cover以及特殊字段在product-content中处理
const PARAM_DEFAULT = {
    title: '',
    title_en: '',
    video: '',
...
const INFO_TYPE = {
    0: '文件$file不存在读取失败, 编辑完成后点击下载按钮, 将文件$file放在目录[public/configuration/product]下',
    1: '文件$file存在变更, 点击下载按钮, 将文件$file放在目录[public/configuration/product]以保存',
    2: '文件$file已存在, 等待合并索引文件product_index.json 后以保存',
    3: '文件$file已是最新且在仓库索引product_index.json 中, 无需保存',
    4: '文件$file已标记为删除, 合并索引文件product_index.json 后生效',
}
...
const mergeFn = () => {
    if (has_unsave.value) {
        ...
        // 下载json
        download_json(merge_index, 'product_index')
    }
}
getIndex().then(res => {
    opt.index_init = res
