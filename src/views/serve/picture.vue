<template>
    <div class="picture">
        ...
        <div class="container">
            ...
            <div class="content">
                <div @click="goPicture(item.id)" class="item" v-for="item in data">
                    <div class="cover">
...
const allTags = computed(() => {
    let tags = {};
    for (const key in INDEX.picture) {
        Object.assign(tags, INDEX.picture[key].tags)
    }
    return tags;
})
...
watch([pagen, tabidx], (to, from) => {
    ...
    dataAll = Object.values(INDEX.picture)?.filter(pic => {
        return Object.keys(pic?.tags)?.some(k => k == tabidx.value)
    })
...
const goPicture = (id) => {
    router.push({ name: 'pictureDetail', params: { id: id }, query: { ...route.query } })
}
</script>
