<template>
    <div ref="dropDom" class="ConfigImage">
        <input type="text" :value="cover" @input="$emit('update:cover', $event.target.value)"
            placeholder=": 填写文件名或拖拽文件到下方">
        <div class="iconfont icon-loading" v-if="loading"></div>
        <div class="box">
            <div v-if="!correct_url" class="cover">
                <span>{{ cover ? '文件不在image_bed文件夹下' : '文件需要放在image_bed文件夹' }}</span>
                <div>[将视频/文件拖拽到此处]</div>
            </div>
            <div v-else class="correctShow">
                <span class="iconfont icon-duigou1"></span>&nbsp;文件存在image_bed文件夹下
            </div>
...
watch(() => props.cover, (to) => {
    ...
    correct_url.value = ''
    if(!to) return;
    const path = `configuration/image_bed/${to}`
    loading.value = true
    getImageBed(to).then(v=>{
        loading.value = false
        if(to !== props.cover) return;
        correct_url.value = path
