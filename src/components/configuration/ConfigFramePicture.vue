<template>
    <div class="ConfigFrame">
        <div v-drag:frameImage.root="frameBackup" class="content">
            <TransitionGroup name="drag-list">
                <div class="tool" v-for="(tool_, idx) in frameBackup" :key="tool_.id">
                    <div @click="tool_.flod = !tool_.flod" class="flodbar">
                        <div @click.stop v-drag:frameImage.target="tool_" class="iconfont icon-tuozhuai"></div>
                        <div class="title">
                            <span>图片{{idx===0?'[第一张为主图/封面]':''}}</span>{{ tool_.raw.src }}
                        </div>
