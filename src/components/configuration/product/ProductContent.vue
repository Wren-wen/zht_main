    ...
    <div class="ArticleContent">
        <div class="editbox scrollBar">
            ...
            <div class="bar flexStart">
                ...
                <config-path v-if="article_edit?.hasOwnProperty('path')" v-model:path="article_edit.path"></config-path>
            </div>
            <div class="bar flexStart">
                <div class="tit marginTop">
                    小字描述:
                </div>
                <config-frame-text v-if="article_edit?.hasOwnProperty('tip')" v-model:frame="article_edit.tip"></config-frame-text>
            </div>
            <div class="bar flexStart">
                <div class="tit marginTop">
                    视频:
                </div>
                <config-file v-if="article_edit?.hasOwnProperty('video')" v-model:cover="article_edit.video"></config-file>
            </div>
            <div class="bar flexStart">
                <div class="tit marginTop">
                    参数图:
                </div>
                <config-image v-if="article_edit?.hasOwnProperty('table')" v-model:cover="article_edit.table"></config-image>
            </div>
            <div class="bar flexStart">
                <div class="tit marginTop">
                    内容:
                </div>
                <config-frame v-if="article_edit?.hasOwnProperty('frame')"
                    v-model:frame="article_edit.frame"></config-frame>
            </div>
            <div class="bar flexStart">
                <div class="tit marginTop">
                    推荐:
                </div>
                <config-frame-text-picture v-if="article_edit?.hasOwnProperty('recommend')" v-model:frame="article_edit.recommend"></config-frame-text-picture>
            </div>
        </div>
        <div class="showbox">
            <config-show>
                <product-detail :product="article_edit"></product-detail>
            </config-show>
        </div>
    </div>
</template>
